...
function onSuccess(position){
    ...
    fetch(`https://api.opencagedata.com/geocode/v1/json?q=${latitude}+${longitude}&key=9c9bed01b25a479e8f6d4e7a8ee578b9`)
    //fetch('https://maps.googleapis.com/maps/api/json?q=${latitude}+${longitude}&key=AIzaSyC9IhxYicIsPOJUdwEcNjMAS00ST_FHyz0')
    //fetch("https://maps.googleapis.com/maps/api/js?q=<latitude><longitude>key=AIzaSyC9IhxYicIsPOJUdwEcNjMAS00ST_FHyz0&callback=initMap&libraries=&v=weekly")
    ...
    let {county, postcode, road, suburb, neighbourhood, state, state_district, country, continent} = allDetails; //getting country, postcode, country properties value from allDetails obj
    button.innerText = `${county}, ${postcode}, ${road}, ${suburb}, ${neighbourhood}, ${state},  ${state_district},${country},    ${continent}`; //passing these value to the button innerText
